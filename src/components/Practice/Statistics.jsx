...
export default function Statistics({ stats, highScores, memory, onClearHistory }) {
  ...
  const accuracy =
    mergedStats.totalNotes > 0 ? (mergedStats.correctNotes / mergedStats.totalNotes) * 100 : 0;

  const averageTime = mergedStats.totalNotes > 0 ? mergedStats.averageTime / 1000 : 0;
  ...
  return (
    ...
    >
      ...
      >
        <StatItem label="总练习次数" value={mergedStats.totalGames} icon="🎯" />
        <StatItem label="最高连击" value={mergedStats.highestCombo} icon="🔥" />
        <StatItem label="准确率" value={`${Math.round(accuracy)}%`} icon="✨" />
        <StatItem label="平均反应时间" value={`${averageTime.toFixed(2)}s`} icon="⚡" />
      </div>
      ...
      >
        ...
        <div style={{ fontSize: '0.85rem', lineHeight: '1.5' }}>
          <div style={{ marginBottom: '0.3rem' }}>
            • <strong>阶段 1</strong>：首次学习 (1 天后复习)
          </div>
          <div style={{ marginBottom: '0.3rem' }}>
            • <strong>阶段 2</strong>：初步巩固 (3 天后复习)
          </div>
          <div style={{ marginBottom: '0.3rem' }}>
            • <strong>阶段 3</strong>：深度记忆 (7 天后复习)
          </div>
          <div style={{ marginBottom: '0.3rem' }}>
            • <strong>阶段 4</strong>：长期记忆 (14 天后复习)
          </div>
          <div>
            • <strong>阶段 5+</strong>：深度掌握 (按需复习)
          </div>
        </div>
      </div>
