import { ALL_NOTES, INTERVALS, CHORDS, ENHARMONIC_FLATS, ENHARMONIC_SHARPS } from '../constants/notes';
...
const sanitizeAllowedNotes = (allowedNotes = []) => {
  ...
  const canonical = allowedNotes
    .map(canonicalizeNote)
    .filter(Boolean);
  return [...new Set(canonical)];
};

const sanitizeFocus = (focusNotes = [], allowedSet) => {
  const canonical = focusNotes
    .map(canonicalizeNote)
    .filter(Boolean);
...
const ensureAllowedCombination = (candidateNotes, allowedSet, allowedNotes, desiredCount, fallbackNote) => {
  if (!allowedSet) {
    return candidateNotes;
  }

  const canonical = candidateNotes
    .map(canonicalizeNote)
    .filter(Boolean);
...
export const generateNotes = (difficulty, options = {}) => {
  ...
  const ensure = (candidates) => ensureAllowedCombination(
    candidates,
    allowedSet,
    allowedNotes,
    desiredCount,
  ...
  switch (difficulty) {
    ...
    case 'INTERMEDIATE': {
      ...
      return ensure([baseNoteString, formatNote(second.pitch, second.octave)]).map(generateNoteDisplay);
    }
...
const sanitizeNoteString = (note) => {
  const parsed = parseNote(note);
  return parsed ? formatNote(parsed.pitch, parsed.octave) : String(note || '').trim();
};
...
const getEnharmonicEquivalent = (noteName) => {
  ...
  // 统一格式：第一个字母大写，其他小写
  const normalized = noteWithoutOctave[0].toUpperCase() + (noteWithoutOctave[1] || '').toLowerCase();
...
export const checkNotes = (expected, actual, options = {}) => {
  ...
  if (ignoreOctave) {
    // 构建计数地图：音符名称 -> 数量
    const buildCountMap = (notes) => notes.reduce((acc, note) => {
  ...
  const compareNotes = (expectedNote, actualNote) => {
    ...
    // 比较音符名称（支持等音体）
    return expParsed.pitch === actParsed.pitch || 
